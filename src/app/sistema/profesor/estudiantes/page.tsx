...
export default function StudentsPage() {
  ...
  const loadEstudiantes = useCallback(async () => {
    ...
    // Tipamos la respuesta explícitamente para evitar 'any'
    const { data, error } = await supabase
      ...
      `) as { data: InscripcionRaw[] | null; error: { message: string } | null };
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-50 font-syne">
      {/* Sidebar visible en todos los tamaños; en móvil se maneja desde el componente */}
      <Sidebar />
