...
import { createClient } from '@/utils/supabase/client'; // Asegúrate de usar tu cliente
import Navbar from '../../components/layout/Navbar';
import Footer from '../../components/layout/Footer';

const RegisterPage = () => {
  const [formData, setFormData] = useState({ name: '', email: '', password: '' });
  const [roleId, setRoleId] = useState<number | null>(null); // Estado para el rol
    ...
    const { error } = await supabase.auth.signUp({
      ...
      options: {
        data: {
          ...
          id_rol: roleId, // Enviamos el rol al trigger
        },
      },
    });
    ...
    <div className="min-h-screen bg-[#f8f9fa] flex flex-col font-syne">
      ...
      <main className="flex-1 flex items-center justify-center p-6">
        <div className="relative z-10 w-full max-w-[1000px] grid grid-cols-1 md:grid-cols-2 bg-white border-4 border-[#1c1917] shadow-[12px_12px_0px_0px_rgba(28,25,23,1)] overflow-hidden">
          ...
          {/* COLUMNA IZQUIERDA: Mensaje en fondo naranja */}
          ...
          <div className="flex flex-col p-8 md:p-12 bg-white">
            ...
            <form className="space-y-5" onSubmit={handleRegister}>
              ...
              <div className="flex gap-4 mb-6">
                <button type="button" onClick={() => setRoleId(2)} className={`flex-1 py-3 border-2 font-black uppercase text-xs ${roleId === 2 ? 'bg-[#f97316] text-white border-[#1c1917]' : 'bg-white border-[#1c1917]'}`}>Profesor</button>
                <button type="button" onClick={() => setRoleId(3)} className={`flex-1 py-3 border-2 font-black uppercase text-xs ${roleId === 3 ? 'bg-[#f97316] text-white border-[#1c1917]' : 'bg-white border-[#1c1917]'}`}>Alumno</button>
              </div>
              ...
              <button disabled={loading} className="w-full bg-[#1c1917] text-white font-black py-4 uppercase border-2 border-[#1c1917] hover:bg-[#f97316] transition-all disabled:opacity-50">
